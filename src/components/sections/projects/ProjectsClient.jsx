...
/* ── Ticker — mix of our services and the real project categories ── */
const TICKER_ITEMS = [
  'Web Development',
  'Business Websites',
    ...
    <article className="group mb-4">
      ...
      <div className="grid lg:grid-cols-12 gap-6 lg:gap-12 items-stretch">
        ...
        <div className={`lg:col-span-6 ${isReversed ? 'lg:order-2' : 'lg:order-2'}`}>
          ...
          >
            <div className={`portfolio-img-clip ${inView ? 'is-revealed' : ''} ${clipDelay} absolute inset-0`}>
              <Image
                fill
                src={project.image}
                alt={project.title}
            ...
            <div className="portfolio-overlay absolute inset-0 z-20 bg-linear-to-t from-foreground/90 via-foreground/40 to-transparent flex flex-col justify-end p-6 sm:p-8">
              <div className="text-white">
                ...
                <h3 className="text-xl sm:text-2xl font-black leading-tight mb-4">{project.shortTitle || project.title}</h3>
        ...
        <div className={`lg:col-span-5 flex flex-col justify-center ${isReversed ? 'lg:order-1' : 'lg:order-3'}`}>
          ...
          <div data-inview={inView ? 'true' : ''} className={`reveal-up ${delay}`}>
            ...
            <Link
              href={detailHref}
              className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-linear-to-r from-primary to-secondary text-white text-sm font-black uppercase tracking-[0.15em] shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 transition-shadow duration-300"
            >
              View Detail
...
export default function ProjectsClient({ data }) {
  ...
  const categories = d.categories?.length ? d.categories : fallbackCategories;
  const hero = d.hero || {};
