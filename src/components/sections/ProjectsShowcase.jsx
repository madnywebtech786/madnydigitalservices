...
const categoryMap = Object.fromEntries(projectCategories.map(c => [c.id, c.name]));
      ...
      <div className={`grid lg:grid-cols-12 gap-6 lg:gap-12 items-stretch`}>
        ...
        <div className={`lg:col-span-6 ${isReversed ? 'lg:order-2' : 'lg:order-2'}`}>
          ...
          >
            ...
            <div
              data-inview={inView ? 'true' : ''}
              className={`reveal-clip ${clipDelays[index % clipDelays.length]} absolute inset-0`}
            >
              <Image
                fill
                src={project.image}
            ...
            <div className="portfolio-overlay absolute inset-0 z-20 bg-linear-to-t from-foreground/90 via-foreground/40 to-transparent flex flex-col justify-end p-8">
              <div className="text-white">
                ...
                <h3 className="text-2xl font-black leading-tight mb-4">{project.title}</h3>
...
export default function ProjectsShowcase({ data }) {
  const d = data || {};
  const projects = (d.items?.filter(p => p.featured) ?? allProjects.filter(p => p.featured)).slice(0, 4);
  ...
  return (
    <section className="py-24 relative overflow-hidden">
      <Container>
        ...
        <div ref={bodyRef}>
          {projects.map((p, i) => (
            <ProjectCard key={p.id} project={p} index={i} inView={bodyInView} />
          ))}
        </div>
